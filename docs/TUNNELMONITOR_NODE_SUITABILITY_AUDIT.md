...
resolved in the pending implementation; integration is still deferred**.
...
- A 128x64 initialize-off operation takes 17 transactions. A full-buffer resync
  at capacity 129 and payload budget 128 takes 42 transactions: 17 init, eight
  repetitions of column address + page address + one data write, then
  `DISPLAY_ON`. The default capacity 65 takes 50 transactions. The subsequent
...
2. Current direct OLED transfers opt into the generic retry/recovery path,
   ...
   future private adapter must make each library callback exactly one physical
   attempt and must not replay an ambiguous display write.
